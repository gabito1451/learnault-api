...
describe('OtpService', () => {
    ...
    beforeEach(() => {
        ...
        mockSend = vi.fn().mockResolvedValue({ success: true, providerMessageId: 'mock_1' })
    ...
    describe('requestChallenge', () => {
        it('revokes prior pending challenges and creates a fresh one', async () => {
            ...
            await otpService.requestChallenge('+2348012345678', 'LOGIN', 'user1', { ip: '1.2.3.4' })
            ...
            expect(mockSend).toHaveBeenCalledWith('+2348012345678', expect.stringContaining('expires in 5 minutes'))
        })
        ...
        it('never sends the raw code hash unhashed to the database', async () => {
            ...
            await otpService.requestChallenge('+2348012345678', 'PHONE_VERIFICATION', 'user1', {})
    ...
    describe('verifyChallenge', () => {
        ...
        it('returns not_found when no pending challenge exists', async () => {
            ...
            const result = await otpService.verifyChallenge('+2348012345678', '123456', 'LOGIN')

            expect(result).toEqual({ ok: false, reason: 'not_found' })
        })

        it('returns not_found when the challenge belongs to a different user (authenticated flow)', async () => {
            ;(prisma.otpChallenge.findFirst as any).mockResolvedValue(buildChallenge({ userId: 'someone-else' }))

            const result = await otpService.verifyChallenge('+2348012345678', '123456', 'PHONE_VERIFICATION', 'user1')
        ...
        it('increments attempts on a wrong code without locking below the threshold', async () => {
            ...
            const result = await otpService.verifyChallenge('+2348012345678', '000000', 'LOGIN')
        ...
        it('consumes the challenge and returns ok on a correct code', async () => {
            ...
            const correctHash = crypto.createHash('sha256').update('+2348012345678:123456').digest('hex')
