...
export type OtpVerifyFailureReason = 'not_found' | 'expired' | 'locked' | 'mismatch'
export type OtpVerifyResult =
  | { ok: true; userId: string }
  | { ok: false; reason: OtpVerifyFailureReason }
...
export class OtpService {
  async requestChallenge(
    ...
    if (!result.success) {
      logger.error(`[OtpService] SMS send failed for phone=${phone}: ${result.error}`)
    }
  }
